# ¡Hola! 👋 Soy Alex 
### Licenciado en Informática | Desarrollador Web

Informático con tres grandes pasiones: el **buen café**, el **Rock** y gritarle a la televisión durante la **Lucha Libre** 🤼. Mi otra habilidad especial: resolver el **Cubo Rubik** más rápido de lo que tardas en cerrar esta ventana. 🧊

---

### 🚀 Sobre mí

- 🎓 **Formación:** Licenciado en Informática.
- 💡 **Enfoque actual:** Especializándome en el desarrollo de lógica de servidor (**Back-End**) y creación de interfaces interactivas (**Front-End**).
- 🌱 **Aprendiendo:** Profundizando en el dominio de **PHP** y **MySQL**, y explorando mejores prácticas en arquitectura de bases de datos y seguridad.
- 🎯 **Objetivo:** Transformar ideas complejas en código eficiente, funcional y fácil de mantener.

---

### 🛠️ Tecnologías y Herramientas

#### **Desarrollo Web (Front & Back)**
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23F7DF1E.svg?style=for-the-badge&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/php-%23777BB4.svg?style=for-the-badge&logo=php&logoColor=white)
![MySQL](https://img.shields.io/badge/mysql-%2300f.svg?style=for-the-badge&logo=mysql&logoColor=white)

#### **Herramientas y Editores**
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/Visual%20Studio%20Code-0078d7.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white)

---

### 📂 Proyectos Destacados

* **ChefEnCuna:** Plataforma de e-learning centrada en la cocina.
  [🔗 Ver Código en GitHub](https://github.com/AlexMayoral221/ChefEnCuna)

---

### 📩 Contáctame

¡Siempre estoy abierto a colaborar en proyectos interesantes o simplemente charlar sobre tecnología (o música)!

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:alexmayoral140@gmail.com)
[![YouTube](https://img.shields.io/badge/YouTube-%23FF0000.svg?style=for-the-badge&logo=YouTube&logoColor=white)](https://www.youtube.com/@alexmayoral221)
[![Discord](https://img.shields.io/badge/Discord-%235865F2.svg?style=for-the-badge&logo=discord&logoColor=white)](https://discord.com/users/shadowmind80)

*Este perfil fue construido con lógica de Cubo, alma de Rock y mucha Informática.*
